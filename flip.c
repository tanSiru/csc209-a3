...
/*
When a child crashes or write to child fails, spawn a new child and reassign the row given by row_index.
*/
void respond_error(int row_index, int width, struct pixel **pixels, struct pixel **new_pixels) {
    // set up pipe for replacement child
    int fd[2][2];
    ...
    if (new_worker > 0) {
        // store new child pid
        child_pid = new_worker;

        // close appropriate pipe ends
        check_close(close(fd[0][0]));
        check_close(close(fd[1][1]));
        ...
        replace_row->row = row_index;
        memcpy(replace_row->pixel_row, pixels[row_index], width * sizeof(struct pixel));

        // write Message to replacement child
        if (write(fd[0][1], replace_row, sizeof(Message) + width * sizeof(struct pixel))== -1) {
            // if write to replacement worker fails, print error and end program
            fprintf(stderr, "reassign row %d to child failed", row_index);  
            exit(1);
        } 

        // close write pipe
        check_close(close(fd[0][1]));

        // read processed row from child
        check_read(read(fd[1][0], replace_row, sizeof(Message) + width * sizeof(struct pixel)));

        // place new row into new pixel array
        new_pixels[row_index] = malloc(width * sizeof(struct pixel));
        check_malloc(new_pixels[row_index]);
        memcpy(new_pixels[row_index], replace_row->pixel_row, width * sizeof(struct pixel));
        
        int new_status;
        if (waitpid(child_pid, &new_status, 0) == -1) {
            perror("wait");
            exit(1);
        }

        check_close(close(fd[1][0]));
        free(replace_row);
    }
...
struct pixel **apply_flip(struct pixel **pixels, int height, int width) {
    ...
    for (int i = 0; i < height; i++) {
        ...
        if (r > 0) {     
            ...
            if (write(fd[i][0][1], init_row, sizeof(Message) + width * sizeof(struct pixel)) == -1) {
                // if write fails, log the error
                fprintf(stderr, "write row %d to child failed. reassigning...", i);

                // spawn a new worker and reassign the row
                respond_error(i, width, pixels, new_pixels);
            }
            ...
            if (worker_count > 9) {
                ...
                if(WIFEXITED(status)) {
                    ...
                    // log error
                    fprintf(stderr, "child for row %d crashed. reassigning row...", i-j+1);

                    // respawn and reassign
                    respond_error(i-j+1, width, pixels, new_pixels);
                }
                }
    ...
    for (int j = worker_count; j > 0; j--) {
        ...
        if (waitpid(child_pids[height-j], &status, 0) == -1) {
            ...
            if(WIFEXITED(status)) {
                ...
                // log error
                fprintf(stderr, "child for row %d crashed. reassigning row...", height-j);

                // respawn and reassign
                respond_error(height-j, width, pixels, new_pixels);
            }
        }
