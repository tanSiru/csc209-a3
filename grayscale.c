...
Message *read_message(int fd, int width){
        ...
        */
        ...
        while(total < size){
                ...
                if(bytes_read == 0){
                        // EOF reached before full message was read → error
                        free(msg);
                        return NULL;
                }
...
struct pixel **apply_grayscale(struct pixel **pixel_array, int height, int width){
        ...
        */ 
        ...
        int max_workers = 3; // maximum # of children that can be created at any given to not blow up the file descriptors through pipe
        ...
        if(modified_pixel_array == NULL){
                perror("malloc");
                exit(1);
        }

        for(int start = 0; start < height; start += max_workers){

                int max_rows = max_workers;
                if(start + max_rows > height){
                        max_rows = height - start;
                }

                int pipe_fd[max_rows][2][2];
                pid_t child_pids[max_rows]; // keep an array of child pids so later we can use waitpid to synchronize wait calls

                for(int k = 0; k < max_rows; k++) {
                        int i = start + k;

                        if(pipe(pipe_fd[k][0])==-1){
                                // ASK HOW WE WILL HANDLE ERROR
                                perror("pipe");
                                exit(1);
                        }

                        if(pipe(pipe_fd[k][1])==-1){
                                perror("pipe");
                                exit(1);
                        }
                        ...
                        if(result>0){
                                close(pipe_fd[k][1][1]); // close the pipe child->parent for writing
                                close(pipe_fd[k][0][0]); // close the pipe parent->child for reading

                                child_pids[k] = result; // store the pid of the child responsible for row i, allowing us to wait for the correct child for pipe_fd[i]
                                ...
                                if(write_message(pipe_fd[k][0][1], row_to_send, width) == -1){
                                        perror("write");
                                        exit(1);
                                }
                                
                                free(row_to_send);
                                close(pipe_fd[k][0][1]); // close write pipe after everything has been written
                        }
                        ...
                        if(result < 0) {
                                ...
                                close(pipe_fd[k][1][0]); // close the pipe child->parent for reading
                                close(pipe_fd[k][0][1]); // close the pipe parent->child for writing

                                

                                // call helper function to handle processing the row and sending the processed row to pipe
                                int child_status = child_process_row(pipe_fd[k][0][0], pipe_fd[k][1][1], width);
                ...
                for(int k = 0; k < max_rows; k++) {
                        ...
                        pid_t childpid = waitpid(child_pids[k], &status, 0);
                        if(childpid == -1){
                                perror("wait");
                        }
                        ...
                        if(WIFEXITED(status) && WEXITSTATUS(status)==0){
                                ...
                                Message *received_msg = read_message(pipe_fd[k][1][0], width);
                                ...
                                */
                                ...
                                if(new_child > 0){
                                        close(pipe_fd[k][1][1]); // close child->parent write
                                        close(pipe_fd[k][0][0]); // close parent->child read

                                        child_pids[k] = new_child;
